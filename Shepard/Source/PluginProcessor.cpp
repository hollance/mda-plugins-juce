...
void MDAShepardAudioProcessor::processBlock(juce::AudioBuffer<float> &buffer, juce::MidiBuffer &midiMessages)
{
  ...
  for (int i = 0; i < buffer.getNumSamples(); ++i) {
    ...
    // buf2 contains a plain sine wave. We multiply this by (rate - 2) in order
    // to fade it in or out as the pitch changes.
    float b =         di  * (buf1[i1] + (rate - 2.0f) * buf2[i1])
            + (1.0f - di) * (buf1[i2] + (rate - 2.0f) * buf2[i2]);

    // Dividing by the rate means we fade out the sound as the pitch goes up,
    // or fade it in when the pitch goes down.
    b *= level / rate;
