...
void MDASubSynthAudioProcessor::update()
{
  ...
  //   y(n + 1) = a * x(n) + (1 - a) * y(n)
  // The coefficient a goes from 0.001 to 0.1. This roughly corresponds to the
  // -3 dB frequency being at 10 - 320 Hz, but I'm not 100% convinced these are
  // correct numbers.
...
void MDASubSynthAudioProcessor::processBlock(juce::AudioBuffer<float> &buffer, juce::MidiBuffer &midiMessages)
{
  ...
  for (int i = 0; i < buffer.getNumSamples(); ++i) {
    ...
    if (type != 3) {
      ...
      // plug-in's effect, move the threshold closer to 0 dB, so that there are
      // fewer of these "pulses".
      if (f2 > threshold) {
        sub = 1.0f;
      ...
      // In Distort mode, the above clipping to produce harmonics is all we do,
      // followed by another series of low-pass filters below. However, for the
      // Divide and Invert modes, we do additional processing.
      ...
      /*
        ...
        The value of sign is initially 1. The distorted signal "sub" is always
        -1, 0, or +1. When sign = 1, nothing happens for sub = 0 or +1, because
        sub * sign is >= 0 in those cases. But when sub becomes -1, sub * sign
        is now < 0 and we enter the if-statement.

        There we flip the value of sign, making it -1. Phase is initially 1.
        Since sign becomes negative, phase becomes -1.

        The output signal is then the distorted signal, which was -1, times the
        value of phase, also -1. So the output is +1.

        While sub remains at -1 (or becomes 0), nothing changes: sub * sign is
        now 0 * -1 or -1 * -1, which both are >= 0.

        At some point, sub becomes +1 again. Now sub * sign is < 0 and we enter
        the if-statement: sign becomes +1, phase remains -1.

        The output signal is sub (+1) times phase (-1), which is -1.

        What happened so far is that the output signal is the inverse of the
        input signal. But that's not all, keep reading...

        While sub remains at +1 (or becomes 0), nothing changes.

        Eventually, sub becomes -1. Now sub * sign is < 0. We flip sign to -1,
        which causes phase to be flipped to +1. The output is now -1 * +1 = -1.

        Note that this two cycles of -1 in a row. Even though the signal went
        from - to + to -, the output stayed at -1.
        ...
        What happens is that the output stays high or low for twice as long as
        the input does. This makes its period twice as large and the frequency
        two times (i.e. one octave) lower.
      */
...
juce::AudioProcessorValueTreeState::ParameterLayout MDASubSynthAudioProcessor::createParameterLayout()
{
  ...
  layout.add(std::make_unique<juce::AudioParameterFloat>(
    ...
    [this](float value, int) {
      // Convert the 0 - 1 parameter into a frequency between 10 and 320 Hz.
      // (It's simpler to just set the NormalisableRange to [10, 320] and give
      // the parameter a skew factor.)
  ...
  layout.add(std::make_unique<juce::AudioParameterFloat>(
    ...
    juce::AudioProcessorParameter::genericParameter,
    [this](float value, int) {
      // Convert the 0 - 1 parameter into a decay value between 0.99 and 1.0.
      float x = 1.0f - std::pow(10.0f, -2.0f - (3.0f * value));

      // For display in the UI, we want to convert this decay value to a time
      // in milliseconds. The original plug-in uses the following formula:
      //     -301.03f / (sampleRate * log10(x))
      // This appears to be an estimate of how long it takes for the signal to
      // decay from 1.0 to roughly 0.001, i.e. by a factor of 1000 or by 60 dB.
      // However, the constant in the formula seems to be off by about 10x.
