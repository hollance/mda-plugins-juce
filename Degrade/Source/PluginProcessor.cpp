...
void MDADegradeAudioProcessor::processBlock(juce::AudioBuffer<float> &buffer, juce::MidiBuffer &midiMessages)
{
  ...
  for (int i = 0; i < buffer.getNumSamples(); ++i) {
    ...
    */
    ...
    // In regular sample mode (mode = 0.0), accum holds only the most recent
    // sample value. If sample-and-hold is active (mode = 1.0), we add the new
    // value to accum so that it holds the sum of the previous samples in this
    // sampling interval. Multiplying by mode (0 or 1) is simpler and possibly
    // faster than using an if-statement.
    accum = in1[i] + in2[i] + mode * accum;

    // Time to take a new sample?
    // The original code used == to check if the sample interval is over, but
    // sampleInterval can change when the user adjusts the parameters. Now the
    // current index may end up being larger than the interval, so use >=.
    if (sampleIndex >= sampleInterval) {

      // Quantize. In sample-and-hold mode, this also takes the mean over the
      // sampling interval (in g1). It also divides by 2 (in g2) to compensate
      // for the stereo -> mono conversion which made the signal twice as loud.
    ...
    // The filter is really 8 identical first-order feedback filters in a row.
    // Each filter does: y(n) = fi * x(n) + fo * y(n - 1), where fi = 1 - fo.
    // Note that in the implementation below only the first filter (b1) and the
    // fifth filter (b6) actually apply the fi coefficient, but the coefficient
    // they're actually using is (1 - fo)^4. This saves some multiplications
    // but is otherwise equivalent. You could also do (1 - fo)^8 and only apply
    // it to b1, not b6, but that can get numerically unstable when fo is large.
    b1 = fi * (x * g3) + fo * b1;
    b2 =       b1      + fo * b2;
    b3 =       b2      + fo * b3;
    b4 =       b3      + fo * b4;
    ...
    // the filter stage. The other b1-b8 are the intermediate output values for
    // the individual filters and also their delay units (z^-1).
