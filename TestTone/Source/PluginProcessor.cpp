...
void MDATestToneAudioProcessor::processBlock(juce::AudioBuffer<float> &buffer, juce::MidiBuffer &midiMessages)
{
  ...
  for (int i = 0; i < buffer.getNumSamples(); ++i) {
    ...
    float x = 0.0f;
      ...
      case 2:
      case 3:
        // The original plug-in assumed RAND_MAX is 32767, but that's usually
      ...
      case 0:
      case 5:
        // Just a simple sine wave oscillator. Increment the phase using the
        // phase increment and wrap around when it reaches 2 pi.
      ...
      case 6:
      case 7:
        // Before the sweep starts, count down and output zeros. This puts two
        // seconds of silence between sweeps.
        if (samplesRemaining > 0) {
          samplesRemaining--;
          phase = 0.0f;
          x = 0.0f;
          ...
          if (mode == 7) {
            // In step mode, round off the frequency value. This steps upwards
            // by roughly 4 semitones (a major third interval).
...
juce::AudioProcessorValueTreeState::ParameterLayout MDATestToneAudioProcessor::createParameterLayout()
{
  ...
  layout.add(std::make_unique<juce::AudioParameterChoice>(
    "Mode",
    "Mode",
    juce::StringArray({ "MIDI #", "IMPULSE", "WHITE", "PINK", "---", "SINE", "LOG SWP.", "LOG STEP", "LIN SWP." }),
    4));
  ...
  layout.add(std::make_unique<juce::AudioParameterFloat>(
    ...
    "L <> R",
    juce::AudioProcessorParameter::genericParameter,
    [](float value, int) { return value > 0.3f ? (value > 0.7f ? "RIGHT" : "CENTRE" ) : "LEFT"; }));
  ...
  layout.add(std::make_unique<juce::AudioParameterFloat>(
    ...
    "dB",
    juce::AudioProcessorParameter::genericParameter,
    [](float value, int) { return value == 0.0f ? "OFF" : juce::String(value, 2); }));
